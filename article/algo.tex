...
Any assignment to $ c_1, c_2, \lambda_1 $ which satisfies this will make a valid $A(x)$
for the first clause. In the same manner, we obtain
...
Now we have $A$'s appearance twice on the second clause, and we try to
solve first by a weaker constraint, which we give allow different
assignments for multiple occurrence. Virtually it is equivalent to
solve the following problem with the templates.
...
By this, duplicating the constraint ~\ref{eq:constr1}, we obtain the
whole constraint as follows.
...
Because the unsatisfiable core contains the constraint that restrict
the parameter $c_3$ or $c_4$ to take different values according to
originating disjunctions. This let us determine to relax the template
to allow a enlarged solution.
...
$G$ if one exists. Later we add modifications to the algorithm if a
