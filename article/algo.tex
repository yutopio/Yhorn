...
\STATE {$visited \gets \left\lbrace v_\bot \right\rbrace$}

\STATE {$C \gets \genConstr (v_\bot, visited)$}
\STATE {$c_{v_\bot} \gets C(v_\bot)$}
\IF {$c_{v_\bot}$ is satisfiable}
  \IF {$visited$ contains all predicate variable vertices}
    \STATE {$visited \gets a$}
    \COMMENT {build solution}
  \ELSE
    \STATE {$visited \gets a$}
  \ENDIF
\ELSE
...
The construction of the algorithm is two fold.
\begin{itemize}
