...
We consider below a Horn graph that is acyclic, and we consider the
case that all disjunction vertices has exactly one predecessor.

For simplicity of discussion, we define \textbf{term arity} $k_v$
for every Horn term vertex $v \in V_T$.
\begin{align*}
k_v =
\begin{cases}
...
\exists a \in V_\vee; \edgel{v}{\theta}{a} \wedge \edgel{a}{\emptyset}{v} \]
...
\qquad \bullet = \substack{
  u \mathop{\leadsto}^\theta v \\
  \exists u' \in V; \xi (u') = u \\
  \hat \Delta (u', \edge{u}{v}) = (\mathbf{a}_{u,U} \mathbf{x}_u \leq b_{u,U}, C_u)} \]
...
We eliminate the restriction that all disjunction vertices $V_\vee$ have
exactly one predecessor. We define the universal set of selectors for
disjunction vertices.
\[ \mathcal{S} = \prod_{v \in V_\vee} \left\lbrace v \mapsto u \middle| \edgel{u}{\emptyset}{v} \right\rbrace \]
