...
A variable mapping label on edges \theta is used to map the index of
...
relation. $v_\bot$ is the greatest element with respect to
$\leadsto^+$.

For each predicate variable $v \in V_T$, the algorithm assigns a
predicate template $\mathbf{a}_v \mathbf{x}_v + b_v \leq 0$ and define
$C_v$ which is a set of linear expressions on template parameters
$\mathbf{a}_v$ and $b_v$.  Intuitively, it denotes the set of
inequalities:
\begin{align*}
\left\lbrace
