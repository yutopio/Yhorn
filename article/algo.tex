...
For each predicate variable vertex, build a linear constraint that
parameters in predicate variable templates must satisfy.  The process
is equivalent to generate a linear constraint in interpolation.

$\genConstr$ procedure produces the constraint for all vertices. The
constraints are first built the weakest so that the disjunctiveness of
