...
\item \textbf{Iterative Constraint Building} The process starts from
  generating the weakest constraint that the input Horn graph allows and
...
\end{itemize}
This algorithm enables a solution to be build as simple as possible.

For the input graph, each predicate variable vertex has a unique
predicate variable label assigned.  All predicate variables are
assigned a simple template.  For a predicate variable $P(\mathbf{x})$,
the algorithm assigns a template $\mathbf{a}_P \mathbf{x} + b_P \leq
0$.  Parameters $\mathbf{a}_P$ and $b_P$ are specified by a linear
constraint.  Then, according to the graph structure, the algorithm
builds a linear constraint that parameters in predicate variable
templates must satisfy.  For constraint building, the algorithm
traverses the graph in the topological order on $V_\Rightarrow$, and
concatenate the constraints from predecessors to build the problem
constraints. Each process to build a constraint on $\mathcal{HC}_v$
for all vertices $v \in V_\Rightarrow$ is equivalent to generate a
linear constraint in interpolation.
