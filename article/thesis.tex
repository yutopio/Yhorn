...
abstraction\cite{DBLP:conf/cav/GrafS97} with CounterExample-Guided
Abstraction Refinement (CEGAR) \cite{DBLP:conf/cav/ClarkeGJLV00}.  An
automated theorem prover finds appropriate predicates for every
...
higher-order programs (MoCHi) \cite{DBLP:conf/pldi/KobayashiSU11}.


\section{Interpolating theorem prover}

Horn clause to interpolation problem.
\cite{DBLP:conf/ppdp/UnnoK09}

CSIsat
\cite{DBLP:conf/cav/BeyerZM08}
...
Lorem ipsum
\cite{DBLP:conf/cav/McMillan03} % Interpolation and SAT-Based Model Checking
\cite{DBLP:conf/popl/HenzingerJMM04} % Abstractions from proofs
\cite{DBLP:journals/tcs/McMillan05} % An interpolating theorem prover
\cite{DBLP:conf/tacas/McMillan05} % Applications of Craig Interpolants in Model Checking
\cite{DBLP:conf/tacas/JhalaM06} % A Practical and Complete Approach to Predicate Refinement
\cite{DBLP:conf/tacas/AmlaM07} % Combining Abstraction Refinement and SAT-Based Model Checking
\cite{DBLP:conf/pldi/BeyerHMR07} % Path invariants
\cite{DBLP:journals/sttt/BeyerHJM07} % The software model checker Blast
\cite{DBLP:conf/vmcai/RybalchenkoS07} % Constraint Solving for Interpolation
\cite{DBLP:conf/cav/Rybalchenko10} % Constraint Solving for Program Verification: Theory and Practice by Example
\cite{DBLP:conf/popl/GuptaPR11} % Predicate abstraction and refinement for verifying multi-threaded programs
\cite{DBLP:conf/aplas/GuptaPR11} % Solving Recursion-Free Horn Clauses over LI+UIF
\cite{DBLP:conf/pepm/SatoUK13} % Towards a scalable software model checker for higher-order


Beautiful interpolants
\cite{DBLP:conf/cav/AlbarghouthiM13}

Disjunctions
\cite{DBLP:conf/cav/RummerHK13}
...
Qunatifiers
\cite{DBLP:conf/sas/BjornerMR13} % On Solving Universally Quantified Horn Clauses
\cite{DBLP:conf/cav/BeyenePR13}. % Solving Existentially Quantified Horn Clauses
