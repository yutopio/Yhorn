...
abstraction\cite{DBLP:conf/cav/GrafS97}.  An automated theorem prover
finds appropriate predicates for specific program locations.  An
execution state at each location is then expressed by a Boolean
valuation of the given predicates.  If the abstraction is too coarse
and a model checker discovers an infeasible error execution path in an
abstract system, the prover computes an additional predicate at a
specific program location to refine the abstraction.  The additional
predicate must be satisfied at the location under the execution in the
concrete model along the discovered path, and the execution does not
lead to the program failure by the same path.  After the refinement,
...
Lorem ipsum
\cite{DBLP:journals/jsyml/Craig57}
\cite{DBLP:conf/cav/GrafS97}
\cite{DBLP:conf/cav/McMillan03}
\cite{DBLP:conf/popl/HenzingerJMM04}
\cite{DBLP:journals/tcs/McMillan05}
\cite{DBLP:conf/tacas/McMillan05}
\cite{DBLP:conf/tacas/JhalaM06}
\cite{website:foci}
\cite{DBLP:conf/tacas/AmlaM07}
\cite{DBLP:conf/pldi/BeyerHMR07}
\cite{DBLP:journals/sttt/BeyerHJM07}
\cite{DBLP:conf/vmcai/RybalchenkoS07}
\cite{DBLP:conf/cav/BeyerZM08}
\cite{DBLP:conf/ppdp/UnnoK09}
\cite{DBLP:conf/cav/Rybalchenko10}
\cite{DBLP:conf/popl/GuptaPR11}
\cite{DBLP:conf/pldi/KobayashiSU11}
\cite{DBLP:conf/aplas/GuptaPR11}
\cite{DBLP:conf/pepm/SatoUK13}
\cite{DBLP:conf/sas/BjornerMR13}
\cite{DBLP:conf/cav/DaiXZ13}
\cite{DBLP:conf/cav/RummerHK13}
\cite{DBLP:conf/cav/AlbarghouthiM13}
\cite{DBLP:conf/cav/BeyenePR13}.
